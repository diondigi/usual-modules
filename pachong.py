...
def SaveNormalRepliesReplies(Comment, Index, Num):
    ...
    # 楼中楼评论API.
    while (1):
        FloorURL = 'https://api.bilibili.com/x/v2/reply/reply?jsonp=jsonp&pn=' + str(
            PageNum) + '&type=1&oid=' + Num + '&ps=10&root=' + str(Comment['data']['replies'][Index]['rpid'])
        Headers = {'Accept': '*/*',
                   'Accept-Encoding': 'gzip, deflate, sdch, br',
                   'Accept-Language': 'zh-CN,zh;q=0.8',
                   'Connection': 'keep-alive',
        ...
        while (1):
            try:
                FileName = '评论区评论.txt'
                with open(FileName, 'a', encoding='utf-8') as F:
                    F.write('\t回复:\t' + CommentFloor['data']['replies'][IndexOfFloor]['member']['uname'] + ':' +
                            CommentFloor['data']['replies'][IndexOfFloor]['content']['message'] + '\n')
                    F.close()
                    IndexOfFloor += 1
